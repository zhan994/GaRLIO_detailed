                                         ...
                                         float inlier_thresh)
{
  ...
  return estimate(radar_scan_msg, v_r, sigma_v_r, inlier_radar_msg, outlier_radar_msg, inlier_thresh);
}
    ...
    if (valid_targets.size() > 2)
    {
      ...
      else
      {
        ...
        if (config_.use_ransac)
        {
          ...
          success = solve3DFullRansac(radar_data, v_r, sigma_v_r, inlier_idx_best, outlier_idx_best, inlier_thresh);
